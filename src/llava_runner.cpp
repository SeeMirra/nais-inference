...
bool LlavaRunner::eval_id(struct llama_context * ctx_llama, int id, int * n_past) {
    std::vector<llama_token> tokens;
    tokens.push_back(id);
    return eval_tokens(ctx_llama, tokens, 1, n_past);
}

bool LlavaRunner::eval_string(struct llama_context * ctx_llama, const char* str, int n_batch, int * n_past, bool add_bos){
    std::string              str2     = str;
...
void LlavaRunner::find_image_tag_in_prompt(const std::string& prompt, size_t& begin_out, size_t& end_out) {
    begin_out = prompt.find(IMG_BASE64_TAG_BEGIN);
    end_out = prompt.find(IMG_BASE64_TAG_END, (begin_out == std::string::npos) ? 0UL : begin_out);
}

bool LlavaRunner::prompt_contains_image(const std::string& prompt) {
    size_t begin, end;
...
llava_image_embed * LlavaRunner::llava_image_embed_make_with_prompt_base64(struct clip_ctx * ctx_clip, int n_threads, const std::string& prompt) {
...
std::string LlavaRunner::remove_image_from_prompt(const std::string& prompt, const char * replacement) {
    size_t begin, end;
    find_image_tag_in_prompt(prompt, begin, end);
...
void LlavaRunner::llava_free(struct llava_context * ctx_llava) {
...
void LlavaRunner::llama_log_callback_logTee(ggml_log_level level, const char * text, void * user_data) {
    (void) level;
    (void) user_data;
    LOG_TEE("%s", text);
}
